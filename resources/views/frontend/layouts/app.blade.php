...
<head>
    ...
    <!-- CSS Files -->
    <link rel="stylesheet" href="{{ static_asset('assets/css/vendors.css') }}?v={{strtotime("now")}}">
    @if(\App\Language::where('code', Session::get('locale', Config::get('app.locale')))->first()->rtl == 1)
    <link rel="stylesheet" href="{{ static_asset('assets/css/bootstrap-rtl.min.css') }}">
    @endif
    <link rel="stylesheet" href="{{ static_asset('assets/css/aiz-core.css') }}?v={{strtotime("now")}}">
    <link rel="stylesheet" href="{{ static_asset('assets/css/custom-style.css') }}?v={{strtotime("now")}}">
...
<body>
    ...
    <script>
        ...
        function show_write_review_modal(order_id)
        {
            $('#order-details-modal-body').html(null);
            ...
            $.post('{{ route('customer.write_review') }}', { _token : AIZ.data.csrf, order_id : order_id}, function(data){
                $('#order-details-modal-body').html(data);
                $('#order_details').modal();
                $('.c-preloader').hide();
            });
