...

{
        ...
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('order_id') != null) {
                ...
                if ($request->payment_option == 'paypal') {
                    ...
                    return $paytm->paytmPayment($order_id, $amount, $user, $phone, $email);
...

    {
        ...
        //check user has not his own product in cart

        $ownerId = [];

        foreach ($carts as $cart) {

            array_push($ownerId, $cart['owner_id']);
        }



        if (count($ownerId) > 0 && in_array(Auth::user()->id, $ownerId)) {

            $key = array_search(Auth::user()->id, $ownerId);

            $sellerSelfProductInCart = Cart::where('owner_id', $ownerId[$key])->first();

            $sellerSelfProduct = Product::where('id', $sellerSelfProductInCart->product_id)->first();

            $sellerSelfProductName = $sellerSelfProduct->name;

            flash(translate(sprintf('Error! Please remove %s from the cart before proceeding. It belongs to your inventory.', $sellerSelfProductName)))->warning();
        ...
        if ($carts && count($carts) > 0) {
            ...
            foreach ($carts as $key => $cartItem) {
                $flashdeal_product = \App\FlashDealProduct::select('flash_deal_id')
                    ...
                    ->first();
                // dd($flashdeal_product);
                if (isset($flashdeal_product['flash_deal_id'])) {
                    /**70% deal */
                    if ($flashdeal_product['flash_deal_id'] == 4) {
                        $deal_2_product[$key]['product_id'] = $cartItem['product_id'];
                        $deal_2_product[$key]['price'] = $cartItem['price'];
                        $deal_2_product[$key]['quantity'] = $cartItem['quantity'];
                        /**599 deal */
                    } elseif ($flashdeal_product['flash_deal_id'] == 3) {
                        $deal_3_product[$key]['product_id'] = $cartItem['product_id'];
                        $deal_3_product[$key]['price'] = $cartItem['price'];
                        $deal_3_product[$key]['quantity'] = $cartItem['quantity'];
                    }
                }
            }
            ...
            foreach ($carts as $key => $cart) {
                $productData = Product::select('unit_price', 'discount as price_discount', 'discount_type')
                    ->where('id', $cart['product_id'])
                    ->first();

                // Check if product data was found
                if ($productData) {
                    // Add the selected keys to the current cart item
                    $carts[$key]['unit_price'] = $productData->unit_price;
                    $carts[$key]['price_discount'] = $productData->price_discount;
                    $carts[$key]['discount_type'] = $productData->discount_type;
                }
            }
